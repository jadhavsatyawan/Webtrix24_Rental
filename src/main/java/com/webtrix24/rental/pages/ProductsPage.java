...
//By dropdownOptions = By.xpath("//div[@class='max-h-60 border bg-white shadow-lg']//div[contains(@class,'cursor-pointer')]//span");
	...
	/*
	 * public void selectProductFromDropdown(String productToSelect) { WebDriverWait
	 * wait = new WebDriverWait(driver, Duration.ofSeconds(15)); JavascriptExecutor
	 * js = (JavascriptExecutor) driver;
	 * 
	 * // Step 1: Click on product name input
	 * wait.until(ExpectedConditions.elementToBeClickable(productName)).click();
	 * 
	 * // Step 2: Optionally refresh list if visible /* try { WebElement refreshBtn
	 * = driver.findElement(By.xpath("//button[contains(text(),'Refresh List')]"));
	 * if (refreshBtn.isDisplayed()) { refreshBtn.click();
	 * System.out.println("🔄 Refreshed product list"); Thread.sleep(2000); // allow
	...
	// if (!found) {
	// throw new RuntimeException("❌ Product not found in dropdown: " +
	// productToSelect);
	// }
	// } */
	...
	/*
	 * public void selectProductType(String productTypeSelect) { WebDriverWait wait
	 * = new WebDriverWait(driver, Duration.ofSeconds(15)); JavascriptExecutor js =
	 ...
	 * 
	 * // Step 2: Optionally refresh list if visible try { WebElement refreshBtn =
	 * driver.findElement(By.xpath("//button[contains(text(),'Refresh List')]")); if
	 * (refreshBtn.isDisplayed()) { refreshBtn.click();
	 * System.out.println("🔄 Refreshed product list"); Thread.sleep(2000); // allow
	 * refresh } } catch (Exception e) {
	 * System.out.println("⚠ Refresh button not found — skipping"); }
	 * 
	 ...
	 * );
	 * 
	 * // Step 4: Select desired product List<WebElement> options =
	 * driver.findElements(dropdownOptions); boolean found = false;
	 * 
	 * for (WebElement option : options) { String optionText =
	 * option.getText().trim(); System.out.println("🔍 Found option: " +
	 * optionText);
	 * 
	 * if (optionText.equalsIgnoreCase(productTypeSelect)) {
	 * js.executeScript("arguments[0].scrollIntoView(true);", option);
	 * option.click(); found = true; System.out.println("✅ Selected product: " +
	 * productTypeSelect); break; } }
	 * 
	 * if (!found) { throw new RuntimeException("❌ Product not found in dropdown: "
	 * + productTypeSelect); } }
	 */

	/*
	 * public void clickUnitField() { WebDriverWait wait = new WebDriverWait(driver,
	 * Duration.ofSeconds(10));
	 * wait.until(ExpectedConditions.elementToBeClickable(unit)); unit.click(); }
	 * 
	 * 
	 * public void selectUnit(String selectUnit) { WebDriverWait wait = new
	 * WebDriverWait(driver, Duration.ofSeconds(15)); JavascriptExecutor js =
	 * (JavascriptExecutor) driver;
	 * 
	 * // Step 1: Click on product name input
	...
	// Action Method
	/*
	 * public void setProductSerialNumber() { // Generate random serial number
...
//Product Serial Search & Select
	public void selectProductBySerial(String serialNumber) throws InterruptedException {
		 ...
		 * 
		 * // Step 3️⃣: Loop through list and select matching serial boolean found =
		 * false; for (WebElement option : ProdropdownOptions) { String text =
		 * option.getText().trim(); System.out.println("📋 Option: " + text);
		 * 
		 * if (text.contains(serialNumber)) {
		 * js.executeScript("arguments[0].scrollIntoView(true);", option);
		 * option.click(); System.out.println("✅ Selected Product: " + serialNumber);
		 * found = true; break; } }
		 * 
		 * // Step 4️⃣: Validation if (!found) { throw new
		 * RuntimeException("❌ Product not found with Serial Number: " + serialNumber);
		 * }
		 * 
		 * } catch (Exception e) {
		 * System.out.println("⚠ Exception in selectProductBySerial: " +
	...
	/*
	 * public void selectRandomProduct() throws InterruptedException { try {
	 * WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));
	 * JavascriptExecutor js = (JavascriptExecutor) driver;
	 * 
	 * // Step 1️⃣: Click the product field ProductRecentActivity.click();
	 * Thread.sleep(500);
	 * 
	 * // Step 2️⃣: Type random letter to trigger list char randomChar = (char) ('a'
	 * + new Random().nextInt(26)); productSearchBox.clear();
	 ...
	 * );
	 * 
	 * if (dropdowns.isEmpty()) { throw new
	 * RuntimeException("❌ No visible dropdown elements found."); }
	 * 
	 * // Step 4️⃣: Randomly pick one visible option Random random = new Random();
	 ...
	 * 
	 * System.out.println("🎯 Randomly selected (JS click): " +
	 * randomElement.getText());
	 * 
	 * } catch (Exception e) {
	 * System.out.println("⚠ Exception in selectRandomProduct (JS mode): " +
	 * e.getMessage()); throw new RuntimeException(e); } }
	 */
...
// ✅ Universal method call (using DropdownUtility)
	public void selectProductSearch(String searchValue) {
		ProductRecentActivity.click();
		// searselectProduct.selectFromSearchableDropdown(driver, productSearchBox,
		// searchValue, ProdropdownOptionsList);
		SearchandselectproductDropdownUtility.selectFromSearchableDropdown(driver, productSearchBox, searchValue,
				ProdropdownOptionsList);
